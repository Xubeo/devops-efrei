	...
	if(isset($_SESSION['username'])){
		...
		if(isset($_GET['action'])){
			...
			if($_GET['action']=='add'){
				...
				if(isset($_POST['submit'])){
					...
					if($title&&$description&&$price&&$stock){
						...
						$insert = $bdd->query("INSERT INTO products (title,slug,description,price,category,weight,shipping,final_price,stock) VALUES('$title','$slug','$description','$price','$category','$weight','$shipping','$final_price_1','$stock')");
			...
			<?php
				...
				$select = $bdd->prepare("SELECT * FROM products WHERE id=$id");
				$select->execute();
				...
				if(isset($_POST['submit'])){
					...
					$update = $bdd->prepare("UPDATE products SET title='$title',description='$description',price='$price',stock='$stock' WHERE id=$id");
					$update->execute();
				...
				if(isset($_POST['submit'])){
					...
					if($name){
						...
						$insert = $bdd->prepare("INSERT INTO category (name,slug) VALUES('$name','$slug')");
						$insert->execute();
				...
				if(isset($_POST['submit'])){
					...
					$select = $bdd->query("SELECT name FROM category WHERE id='$id'");

					$result = $select->fetch(PDO::FETCH_OBJ);

					$update = $bdd->prepare("UPDATE category SET name='$name' WHERE id=$id");
					$update->execute();

					$id = $_GET['id'];
				
					$update = $bdd->query("UPDATE products SET category='$name' WHERE category='$result->name'");
				...
				if(isset($_POST['submit2'])){
					...
					if($tva){
						...
						$update = $bdd->query("UPDATE products SET tva=$tva");
						header("Refresh:0");
