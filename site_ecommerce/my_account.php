...
$select = $bdd->query("SELECT * FROM users WHERE id = '$user_id'");
...
<?php
$select = $bdd->query("SELECT * FROM transactions WHERE user_id = '$user_id'");

while($s = $select->fetch(PDO::FETCH_OBJ)){

	$transaction_id = $s->transaction_id;
	$select2 = $bdd->query("SELECT * FROM products_transactions WHERE transaction_id='$transaction_id'");
