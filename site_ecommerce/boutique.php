<?php
	...
	if(isset($_GET['show'])){
		...
		$select = $bdd->prepare("SELECT * FROM products WHERE slug='$product'");
		$select->execute();	
	...
	if(isset($_GET['category'])){
		...
		$select = $bdd->query("SELECT name FROM category WHERE slug='$category_slug'");
		...
		while($s=$select->fetch(PDO::FETCH_OBJ)){
			...
			?>
			<br/>
			<a class="image" href="?show=<?php echo $s->slug; ?>"><img src="imgs/<?php echo $s->slug; ?>.jpg"/></a>
